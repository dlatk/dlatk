...
from FeatureWorker.fwConstants import DEF_ENCODING, MAX_SQL_PRINT_CHARS
...
def executeGetSSCursor(db, sql, warnMsg = True, charset=DEF_ENCODING):
    ...
    ssCursor = dbConnect(db, charset=charset)[0].cursor(MySQLdb.cursors.SSCursor)
    data = []
    attempts = 0;
    while (1):
        try:
...
def getTableColumnNames(db, table, charset=DEF_ENCODING):
    """Returns a list of column names from a db table"""
    (dbConn, dbCursor, dictCursor) = dbConnect(db, charset=charset)
...
def getTableColumnNamesTypes(db, table, charset=DEF_ENCODING):
    ...
    return executeGetList(db, dbCursor, sql, charset=charset)
...
def execute( db, dbCursor, sql, warnQuery=True, charset=DEF_ENCODING):
...
def qExecute( db, sql, warnQuery=False, charset=DEF_ENCODING):
    """performs the db connect and execute in the same call"""
    (dbConn, dbCursor, dictCursor) = dbConnect(db, charset=charset)    
    return execute( db, dbCursor, sql, warnQuery, charset=charset)

def executeGetDict( db, dictCursor, sql, warnQuery=False, charset=DEF_ENCODING):
...
def executeGetList( db, dbCursor, sql, warnQuery=True, charset=DEF_ENCODING):
...
def executeGetList1( db, dbCursor, sql, warnQuery=False, charset=DEF_ENCODING):
    """Executes a given query, expecting one resulting column. Returns results as a list"""
    return map(lambda x:x[0], executeGetList( db, dbCursor, sql, warnQuery, charset=charset))

def qExecuteGetList( db, sql, warnQuery=False, charset=DEF_ENCODING):
    """performs the db connect and execute in the same call"""
    (dbConn, dbCursor, dictCursor) = dbConnect(db, charset=charset)    
    return executeGetList( db, dbCursor, sql, warnQuery, charset=charset)

def qExecuteGetList1( db, sql, warnQuery=False, charset=DEF_ENCODING):
    """performs the db connect and execute in the same call, equivalent to executeGetList1"""
    (dbConn, dbCursor, dictCursor) = dbConnect(db, charset=charset)    
    return executeGetList1( db, dbCursor, sql, warnQuery, charset=charset)

def executeWrite( db, dbConn, sql, row, writeCursor=None , warnQuery=False, charset=DEF_ENCODING):
...
def doesTableExist( db, table, charset=DEF_ENCODING ):
    sql = 'show tables in %s'%db
    tables = qExecuteGetList1(db, sql, charset=charset)
    return table in tables

def executeWriteMany( db, dbConn, sql, rows, writeCursor=None, warnQuery=False, charset=DEF_ENCODING):
    """Executes a write query"""
    if warnQuery:
        warn("SQL (write many) QUERY: %s"% sql)                                                 
    if not writeCursor:
...
def qExecuteWriteMany( db, sql, rows, writeCursor=None, warnQuery=False, charset=DEF_ENCODING):
    """Executes a write query"""
    (dbConn, dbCursor, dictCursor) = dbConnect(db, charset=charset)
    return executeWriteMany(db, dbConn, sql, rows, writeCursor, warnQuery, charset=charset)
...
def getColumnNamesAndTypes( db, table, charset=DEF_ENCODING ):
    """returns a dict of column names mapped to types"""
    sql = """SELECT column_name, column_type FROM information_schema.columns where TABLE_SCHEMA = '%s' AND TABLE_NAME = '%s'"""%(db, table)
    return dict(qExecuteGetList(db, sql, charset=charset))


def cloneExactTable(db, sourceTableName, destinationTableName, charset=DEF_ENCODING):
    ...
    execute(db, dbCursor, cloneQuery, True, charset=charset)
    ...
    execute(db, dbCursor, populateQuery, True, charset=charset)

    warn("finished cloning table!")

def randomSubsetTable( db, sourceTableName, destinationTableName, keyField, percentToSubset=.10, distinct=True, charset=DEF_ENCODING):
    ...
    execute(db, dbCursor, sql, True, charset=charset)
    ...
    uniqueKeyList = executeGetList1(db, dbCursor, sql, True, charset=charset)
    ...
    execute(db, dbCursor, populateQuery, False, charset=charset)
...
def writeTableToCSV(db, tableName, outputfile, sql_extra=None, charset=DEF_ENCODING):
    (cur, conn, dcur) = dbConnect(db, charset=charset)
    sql = 'SELECT * FROM %s'%tableName
    if sql_extra:
        sql += ' ' + sql_extra
    dictRows = executeGetDict(db, dcur, sql, charset=charset)
    if dictRows:
...
## TABLE MAINTENANCE ##
def optimizeTable(db, dbCursor, table, charset=DEF_ENCODING):
    """Optimizes the table -- good after a lot of deletes"""
    sql = """OPTIMIZE TABLE %s """%(table)
    return execute(db, dbCursor, sql, charset=charset) 

def disableTableKeys(db, dbCursor, table, charset=DEF_ENCODING):
    """Disable keys: good before doing a lot of inserts"""
    sql = """ALTER TABLE %s DISABLE KEYS"""%(table)
    return execute(db, dbCursor, sql, charset=charset) 

def enableTableKeys(db, dbCursor, table, charset=DEF_ENCODING):
    """Enables the keys, for use after inserting (and with keys disabled)"""
    sql = """ALTER TABLE %s ENABLE KEYS"""%(table)
    return execute(db, dbCursor, sql, charset=charset) 

## Table Meta Info ##
def tableExists(db, dbCursor, table, charset=DEF_ENCODING):
    sql = """show tables like '%s'""" % table
    if executeGetList(db, dbCursor, sql, charset=charset):
        return True
    else:
        return False

def getTableDataLength(db, dbCursor, table, charset=DEF_ENCODING):
    ...
    return executeGetList(db, dbCursor, sql, charset=charset)[0]

def getTableIndexLength(db, dbCursor, table, charset=DEF_ENCODING):
    """Returns the data length for the given table"""
    sql = """SELECT INDEX_LENGTH FROM information_schema.tables where TABLE_SCHEMA = '%s' AND TABLE_NAME = '%s'""" % (db, table)
    return executeGetList(db, dbCursor, sql, charset=charset)[0]

def getTableColumnNameTypes(db, dbCursor, table, charset=DEF_ENCODING):
    """returns a dict of column names mapped to types"""
    sql = """SELECT column_name, column_type FROM information_schema.columns where TABLE_SCHEMA = '%s' AND TABLE_NAME = '%s'"""%(db, table)
    return dict(executeGetList(db, dbCursor, sql, charset=charset))


def getTableColumnNameList(db, dbCursor, table, charset=DEF_ENCODING):
    """returns a dict of column names mapped to types"""
    ...
    return [x[0] for x in executeGetList(db, dbCursor, sql, charset=charset)]
