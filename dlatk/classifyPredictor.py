...
class ClassifyPredictor:
    ...
    backOffPerc = 0.00  # when the num_featrue / training_insts is less than this backoff to backoffmodel
    ...
    featureSelectMin = (
        50  # must have at least this many features to perform feature selection
    )
    ...
    def train(
        self, standardize=True, sparse=False, restrictToGroups=None, groupsWhere=""
    ):
        ...
        if restrictToGroups:  # restrict to groups
            ...
            if isinstance(restrictToGroups, dict):
                rGroups = [
                    item
                    for sublist in list(restrictToGroups.values())
                    for item in sublist
                ]
    ...
    def test(
        ...
        if sparse:
            ...
            (
                groupNorms,
                featureNames,
            ) = self.featureGetter.getGroupNormsWithZerosFeatsFirst(
        ...
        for outcomeName, outcomes in sorted(allOutcomes.items()):
            ...
            print(
                "*Pearson r:                          %.4f (p = %.5f)"
                % pearsonr(ytest, ypred)
            )
            print(
                "*Spearman rho:                       %.4f (p = %.5f)"
                % spearmanr(ytest, ypred)
            )
    ...
    def testControlCombos(
        ...
        (
            ...
            allControls,
            foldLabels,
        ) = self.outcomeGetter.getGroupsAndOutcomes(
            ...
            for controlKeyCombo in combinations(controlKeys, r):
                ...
                for outcomeName, outcomes in sorted(allOutcomes.items()):
                    ...
                    for withLanguage in range(2):
                        ...
                        for testChunk in [
                            i
                            for i in range(0, len(groupFolds))
                            if not testFolds or i in testFolds
                        ]:
                            trainGroups = set()
                            for chunk in (
                                groupFolds[:testChunk] + groupFolds[(testChunk + 1) :]
                            ):
                            ...
                            for i in gnListIndices:
                                ...
                                (Xdicts, ydict) = (groupNormValues, outcomes)
                                print(
                                    "   (feature group: %d): [Initial size: %d]"
                                    % (i, len(ydict))
                                )
                                (Xtrain, ytrain) = alignDictsAsXy(
                                    Xdicts, ydict, sparse=True, keys=trainGroupsOrder
                            ...
                            predictionProbs.update(
                                dict(zip(testGroupsOrder, ypredProbs))
                            )
                            ...
                            auc = computeAUC(
                                ytest, ypredProbs, multiclass, negatives=False
                            )
                        ...
                        for k, v in list(testStats.items()):
                            if isinstance(v, list):
                                ...
                                reportStats["folds_se_" + k] = std(v) / sqrt(
                                    float(nFolds)
                                )
                            else:
                                reportStats[k] = v
                        ...
                        reportStats["precision"] = precision_score(
                            ytrue, ypred, average="macro"
                        )
                        reportStats["recall"] = recall_score(
                            ytrue, ypred, average="macro"
                        )
                        ...
                        reportStats["mfclass_acc"] = testCounter[mfclass] / float(
                            len(ytrue)
                        )
                        ...
                        if controlCombineProbs:  # ensemble with controls (AUC weighted)
                            ...
                            if withLanguage and savedControlYpp is not None:
                                newProbsDict = ensembleNFoldAUCWeight(
                                    outcomes,
                                    [predictionProbs, savedControlPProbs],
                                    groupFolds,
                                )
                                ...
                                reportStats["auc_cntl_comb2"] = computeAUC(
                                    ensYtrue, np.array(ensYPredProbs), multiclass
                                )
                                (
                        ...
                        print(
                            "       + precision: %.4f (+- %.4f)"
                            % (
                                reportStats["folds_precision"],
                                reportStats["folds_se_precision"],
                            )
                        )
                        print(
                            "       +    recall: %.4f (+- %.4f)"
                            % (
                                reportStats["folds_recall"],
                                reportStats["folds_se_recall"],
                            )
                        )
                        if saveModels:
                            print(
                                "!!SAVING MODELS NOT IMPLEMENTED FOR testControlCombos!!"
                            )
                        try:
                            scores[outcomeName][controlKeyCombo][
                                withLanguage
                            ] = reportStats
                        except KeyError:
                            scores[outcomeName][controlKeyCombo] = {
                                withLanguage: reportStats
                            }
    ...
    def predict(
        ...
        for outcomeName, outcomes in sorted(allOutcomes.items()):
            ...
            if isinstance(restrictToGroups, dict):  # outcome specific restrictions:
                outcomes = dict(
                    [
                        ...
                        if g in restrictToGroups[outcomeName]
                    ]
                )
            ...
            for i in range(
                len(groupNormsList)
            ):  # get each feature group data (i.e. feature table)
                groupNormValues = groupNormsList[i]
                (Xdicts, ydict) = (groupNormValues, outcomes)
                print("  (feature group: %d)" % (i))
                (Xtest, ytest) = alignDictsAsXy(
            ...
            print(
                " *ACC: %.4f (mfclass_acc: %.4f); mfclass: %s"
            ...
            mse = metrics.mean_squared_error(ytest, ypred)
            print("*Mean Squared Error:                 %.4f" % mse)
            assert len(thisTestGroupsOrder) == len(
                ypred
            ), "can't line predictions up with groups"
            if probs:
                predictions[outcomeName] = dict(
                    list(zip(thisTestGroupsOrder, ypredProbsMaxClass))
                )
            else:
    ...
    def predictNoOutcomeGetter(
        self, groups, standardize=True, sparse=False, restrictToGroups=None
    ):
        ...
        for outcomeName in sorted(outcomes):
            ...
            for i in range(
                ...
                groupNormValues = groupNormsList[
                    i
                ]  # basically a feature table in list(dict) form
    ...
    def predictAllToFeatureTable(
      ...
      ERROR: predictToFeatureTable doesn't work when --outcome_table
        ...
        if len(groups) > self.maxPredictAtTime:
            ...
            chunks = [
                x for x in foldN(groups, int(ceil(len(groups) / self.maxPredictAtTime)))
            ]
        predictions = dict()
        totalPred = 0
        ...
        for chunk in chunks:
            ...
            for feat in featNames:
                ...
                # pprint(preds)#DEBUG
                print(
                    "[Inserting Predictions as Feature values for feature: %s]" % feat
                )
                wsql = (
                    """INSERT INTO """
                    + featureTableName
        ...
        for i, featTableFeats in enumerate(self.featureNamesList):
            ...
            for outcome, model in self.classificationModels.items():
                ...
                if np.all(coefficients == 0):
                    warn(
                        "WARNING: All coefficients equal to zero for outcome %s"
                        % outcome
                    )
                    continue
                ...
                if self.multiFSelectors[outcome][i]:
                    print(
                        "Inverting the feature selection: %s"
                        % self.multiFSelectors[outcome][i]
                    )
                    coefficients = self.multiFSelectors[outcome][i].inverse_transform(
                        coefficients
                    )
                ...
                coefficients = np.asarray(
                    [list(islice(coeff_iter, 0, j)) for j in self.featureLengthList][i]
                )
                ...
                if outcome not in intercept_dict:
                    ...
                    print(
                        "Intercept for {o} [{i}]".format(
                            o=outcome, i=intercept_dict[outcome]
                        )
                    )
                print(
                    "coefficients size for {f}: {s}".format(
                        f=featTables[i], s=coefficients.shape
                    )
                )
        ...
        if unpackTopicWeights:
            ...
            if len(unpackTopicWeights) == len(weights_dict):
                ...
                if len(unpackTopicWeights) == 1:
                    ...
                    print(
                        "Unpacking {topicFeatTable}".format(
                            topicFeatTable=topicFeatTable
                        )
                    )
                ...
                for idx, topicFeatTable in enumerate(unpackTopicWeights):
                    ...
                    for featTable in weights_dict:
                        if featTable.split("$")[1].startswith(buildTable):
                            for outcome, wordDict in weights_dict[
                                topicFeatTable
                            ].items():
                                for word, weight in wordDict.items():
    ...
    def _train(self, X, y, standardize=True):
        ...
        if (
            X.shape[1] / float(X.shape[0])
        ) < self.backOffPerc:  # backoff to simpler model:
            print(
                "number of features is small enough, backing off to %s"
                % self.backOffModel
            )
    ...
    def _multiXtrain(
        ...
        while i < len(multiX):  # changed to while loop by Youngseo
            ...
            if standardize == True:
                ...
                if self.outliersToMean and not sparse:
                    ...
                    print(
                        "  [Setting outliers (> %d) to mean for X[%d]]"
                        % (self.outliersToMean, i)
                    )
                y = np.array(y)
            elif self.outliersToMean:
                print(
                    " Warning: Outliers to mean is not being run because standardize is off"
                )
            ...
            if self.featureSelectionString and X.shape[1] >= self.featureSelectMin:
                ...
                if self.featureSelectPerc < 1.0:
                    ...
                    _, Xsub, _, ysub = train_test_split(
                        X,
                        y,
                        test_size=self.featureSelectPerc,
                        train_size=1,
                        random_state=0,
                    )
                    fSelector.fit(Xsub, ysub)
        ...
        if hasMultValuesPerItem(self.cvParams[modelName]) and modelName[-2:] != "cv":
            ...
            if len(classes) > 2:
                parameters = {
                    "estimator__" + k: v for k, v in self.cvParams[modelName][0].items()
                }
    ...
    def roc(
        ...
        for outcomeName, outcomes in sorted(allOutcomes.items()):
            ...
            (
                ...
                multiXtrain, ytrain, standardize, sparse=sparse, output_name=output_name
            )
    ...
    def _roc(self, X, y, standardize=True, sparse=False, output_name=None):
        ...
        if hasMultValuesPerItem(self.cvParams[modelName]) and modelName[-2:] != "cv":
            ...
            for i in range(y_train.shape[1]):
                ...
                print(
                    "[Performing grid search for parameters over training for class: %d]"
                    % i
                )
    ...
    def roc_curves(self, y_test, y_score, output_name=None):
        ...
        try:
            pp = PdfPages(
                output_name + ".pdf" if output_name[-4:] != ".pdf" else output_name
            )
        except NameError:
    ...
    def _multiXpredict(
        ...
        while i < len(multiX):  # changed to while loop by Youngseo
            ...
            if scaler:
                print(
                    (
                        "  predict: applying standard scaler to X[%d]: %s"
                        % (i, str(scaler))
                    )
                )  # debug
                try:
            ...
            if fSelector:
                print(
                    (
                        "  predict: applying feature selection to X[%d]: %s"
                        % (i, str(fSelector))
                    )
                )  # debug
                newX = fSelector.transform(X)
    ...
    def load(self, filename, pickle2_7=True):
        ...
        try:
            print(
                "Outcomes in loaded model:",
                list(tmp_dict["classificationModels"].keys()),
            )
        except KeyError:
    ...
    def printComboControlScoresToCSV(
        ...
        for outcomeName in outcomeKeys:
            ...
            csvOut = csv.DictWriter(
                outputstream, fieldnames=columnNames, delimiter=delimiter
            )
    ...
    @staticmethod
    def printComboControlPredictionsToCSV(
        ...
        for outcomeName in scores:
            for rk in scores[outcomeName]:
                for withLang in scores[outcomeName][rk]:
                    groups.update(
                        [k for k in scores[outcomeName][rk][withLang]["predictions"]]
                    )
        ...
        for outcomeName in outcomeKeys:
            ...
            for rk in rowKeys:
                for withLang, s in outcomeScores[rk].items():
                    ...
                    predictionData[str(i) + "_" + outcomeName + "_" + mc] = s[
                        "predictions"
                    ]
            ...
            for rk in scores[outcomeName]:
                for withLang in scores[outcomeName][rk]:
                    groups.update(
                        [
                            k
                            for k in scores[outcomeName][rk][withLang][
                                "predictionProbs"
                            ]
                        ]
                    )
    ...
    def old_train(self, standardize=True, sparse=False, restrictToGroups=None):
        ...
        if len(self.featureGetters) > 1:
            print(
                "WARNING: multiple feature tables passed in rp.train only handles one for now."
            )
        if sparse:
            (
                groupNorms,
    ...
    def old_predict(self, standardize=True, sparse=False, restrictToGroups=None):
        ...
        if len(self.featureGetters) > 1:
            print(
                "WARNING: multiple feature tables passed in rp.predict only handles one for now."
            )
        if sparse:
            (
                groupNorms,
        ...
        for outcomeName, outcomes in allOutcomes.items():
            ...
            if self.chunkPredictions:
                ...
                for subX, suby in chunks(X, ytest, self.maxPredictAtTime):
                    ysubpred = self._predict(
                        classifier, subX, scaler=scaler, fSelector=fSelector
                    )
...
def stratifyGroups(groups, outcomes, folds, randomState=DEFAULT_RANDOM_SEED):
    ...
    for c, gs in outcomesByClass.items():
        foldsOrder = [
            x[0] for x in sorted(list(countPerFold.items()), key=lambda x: (x[1], x[0]))
        ]
...
def matrixAppendHoriz(A, B):
    if isinstance(A, spmatrix) and isinstance(B, spmatrix):
        ...
        raise ValueError(
            "matrix append types not supported: %s and %s"
...
def ensembleNFoldAUCWeight(outcomes, probsListOfDicts, groupFolds):
    ...
    for testChunk in [i for i in range(0, len(groupFolds))]:
        ...
        (Xtrain, ytrain) = alignDictsAsXy(
            probsListOfDicts, outcomes, keys=trainGroupsOrder
        )
        (Xtest, ytest) = alignDictsAsXy(
            probsListOfDicts, outcomes, keys=testGroupsOrder
        )
        ...
        for c in range(Xtrain.shape[1]):
            ...
            weights[c] = (
                (max(computeAUC(ytrain, Xtrain[:, c, :], negatives=False), 0.5) - 0.5)
                / 0.5
            ) ** 2
...
def easyNFoldAUCWeight(X, y, folds):
    ...
    groupFolds = stratifyGroups(
        groups, outcomes, folds, randomState=DEFAULT_RANDOM_SEED
    )
    ymap = []
    ...
    for testgs in groupFolds:
        ...
        for c in range(Xtrain.shape[1]):
            weights[c] = (
                (np.absolute(pos_neg_auc(ytrain, Xtrain[:, c])) - 0.5) / 0.5
            ) ** 2
            sumWeights += weights[c]
        weights = np.array([weights / sumWeights])
        warn(weights)  # debug
