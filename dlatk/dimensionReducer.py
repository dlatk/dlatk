    ...
    def fit(self, standardize=False, sparse=False, restrictToGroups=None):
        ...
        if allOutcomes:
            for outcomeName, outcomes in allOutcomes.items():
                ...
                ) = self._fit(X, y, standardize=standardize)
        else:
            ...
            ) = self._fit(X, standardize=standardize)
            ...
            for i in range(n):
                ...
                for j in range(m):
                    if abs(component_mat[i][j]) > 0:
                        ...
                        reduction_dict["component_" + str(i)][self.featureNames[j]] = component_mat[
                            i
                        ][j]
...
class PPA(TransformerMixin, BaseEstimator):
    ...
    def fit(self, X: np.array, y=None):
        ...
        if self.D is None:
            self.D = max(X.shape[1] // 100, 1)  # init D if not passed
...
class NonNegativeStandardScaler(TransformerMixin, BaseEstimator):
    ...
    def transform(self, X: np.ndarray, y=None):
        ...
        if X[X < 0].shape[0]:
            print(
                "Matrix with %d negative values after applying axis shifting. Zeroing those values now...."
                % (X[X < 0].shape[0])
            )
        X[X < 0] = 0
        return X
...
class CCA:
    ...
    def ccaPermuteOutcomesVsControls(self, nPerms=25, penaltyXs=None, penaltyZs=None):
        ...
        kwParams["penaltyxs"] = (
            penaltyXs if penaltyXs else ro.vectors.FloatVector(np.arange(0.1, 0.91, 0.05))
        )
        kwParams["penaltyzs"] = (
            penaltyZs if penaltyZs else ro.vectors.FloatVector(np.arange(0.1, 0.91, 0.05))
        )
