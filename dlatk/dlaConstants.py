...
DEF_RP_FEATURE_SELECTION_MAPPING = {
    ...
    'topic_ngram_ms': 'Pipeline([("1_mean_value_filter", OccurrenceThreshold(threshold=int(sqrt(X.shape[0]*10000)))), ("2_univariate_select", SelectFwe(f_regression, alpha=60.0)), ("3_rpca", RandomizedPCA(n_components=max(int(X.shape[0]*0.08*(log2(100000) - log2(X.shape[1] - X.shape[0]))), min(50, X.shape[1])), random_state=42, whiten=False, iterated_power=3))])',
