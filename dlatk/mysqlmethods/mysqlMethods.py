...
from dlatk.dlaConstants import (
    USER,
    MAX_ATTEMPTS,
    MYSQL_ERROR_SLEEP,
    MYSQL_HOST,
...
def executeGetSSCursor(
    db,
    sql,
    warnMsg=True,
    charset=DEF_ENCODING,
    use_unicode=DEF_UNICODE_SWITCH,
    host=MYSQL_HOST,
):
    ...
    ssCursor = dbConnect(db, charset=charset, use_unicode=use_unicode, host=host,)[
        0
    ...
    while 1:
        try:
            ...
            ssCursor = dbConnect(db, charset=charset, use_unicode=use_unicode)[
                0
            ].cursor(MySQLdb.cursors.SSCursor)
...
def dbConnect(
    db, host=MYSQL_HOST, charset=DEF_ENCODING, use_unicode=DEF_UNICODE_SWITCH
):
    ...
    while 1:
        try:
            if os.environ.get("SSH_HOST") is not None:
...
def abstractDBConnect(db, host=MYSQL_HOST, user=USER):
    dbConn = MySQLdb.connect(host=host, user=user, db=db)
    dbCursor = dbConn.cursor()
    return (dbConn, dbCursor)


def getTableColumnNames(
    db, table, charset=DEF_ENCODING, use_unicode=DEF_UNICODE_SWITCH
):
    """Returns a list of column names from a db table"""
    (dbConn, dbCursor, dictCursor) = dbConnect(
        db, charset=charset, use_unicode=use_unicode
    )
    sql = (
...
def qExecute(
    db, sql, warnQuery=False, charset=DEF_ENCODING, use_unicode=DEF_UNICODE_SWITCH
):
    ...
    return execute(
        db, dbCursor, sql, warnQuery, charset=charset, use_unicode=use_unicode
    )
...
def getTableEncoding(
    db, table=None, charset=DEF_ENCODING, use_unicode=DEF_UNICODE_SWITCH
):
    """returns the encoding of a given table as a string"""
    if table:
        sql = (
            """SELECT CCSA.character_set_name FROM information_schema.`TABLES` T, information_schema.`COLLATION_CHARACTER_SET_APPLICABILITY` CCSA WHERE CCSA.collation_name = T.table_collation AND T.table_schema = '%s' AND T.table_name = '%s' """
            % (db, table)
        )
    else:
        sql = (
...
def cloneExactTable(
    ...
    warn(
        "making TABLE %s, an exact copy of TABLE %s..."
...
def randomSubsetTable(
    db,
    sourceTableName,
    destinationTableName,
    keyField,
    percentToSubset=0.10,
    distinct=True,
    charset=DEF_ENCODING,
    use_unicode=DEF_UNICODE_SWITCH,
):
    warn(
        "making TABLE %s, a %2.2f percent random subset of TABLE %s on unique key %s..."
        % (destinationTableName, percentToSubset, sourceTableName, keyField)
    )
    ...
    warn(
        "grabbing a%s subset (%2.6f percent) of the keys on which to base the new table"
        % (isDistinctText, 100 * percentToSubset)
    )
    sql = (
        "SELECT DISTINCT(%s) FROM %s" % (keyField, sourceTableName)
        if distinct
        else "SELECT %s FROM %s" % (keyField, sourceTableName)
    )
    uniqueKeyList = executeGetList1(
        db, dbCursor, sql, True, charset=charset, use_unicode=use_unicode
    )
    ...
    warn("populating newly created table, TABLE %s" % (destinationTableName))
    populateQuery = "INSERT INTO %s SELECT * FROM %s WHERE %s IN (%s)" % (
        destinationTableName,
        sourceTableName,
        keyField,
        ",".join(newKeys),
    ...
    execute(
        db, dbCursor, populateQuery, False, charset=charset, use_unicode=use_unicode
    )

    warn(
        "finished making TABLE %s, a %2.2f percent random subset of TABLE %s on unique key %s!"
        % (destinationTableName, percentToSubset, sourceTableName, keyField)
    )
...
def writeTableToCSV(
    ...
    if dictRows:
        csvOut = csv.DictWriter(
            open(outputfile, "w"), fieldnames=list(dictRows[0].keys())
        )
        csvOut.writeheader()
...
## TABLE MAINTENANCE ##
def optimizeTable(
    db, dbCursor, table, charset=DEF_ENCODING, use_unicode=DEF_UNICODE_SWITCH
):
...
def standardizeTable(
    db,
    dbCursor,
    table,
    collate="",
    engine=DEF_MYSQL_ENGINE,
    charset=DEF_ENCODING,
    use_unicode=DEF_UNICODE_SWITCH,
):
    """Sets character set, collate and engine"""
    if not collate:
        collate = DEF_COLLATIONS[charset.lower()]
    sql = """ALTER TABLE %s CHARACTER SET %s COLLATE %s ENGINE=%s""" % (
        table,
        charset,
        collate,
        engine,
...
def primaryKeyExists(
    ...
    if executeGetList(
        db, dbCursor, sql, warnQuery=False, charset=charset, use_unicode=use_unicode
    ):
        return True
    else:
        return False
...
def getTableIndexLength(
    db, dbCursor, table, charset=DEF_ENCODING, use_unicode=DEF_UNICODE_SWITCH
):
    """Returns the data length for the given table"""
    sql = (
        """SELECT INDEX_LENGTH FROM information_schema.tables where TABLE_SCHEMA = '%s' AND TABLE_NAME = '%s'"""
        % (db, table)
    )
    return executeGetList(db, dbCursor, sql, charset=charset, use_unicode=use_unicode)[
        0
...
def getTableColumnNameTypes(
    ...
    return dict(
        executeGetList(db, dbCursor, sql, charset=charset, use_unicode=use_unicode)
    )


def getTableColumnNameList(
    db, dbCursor, table, charset=DEF_ENCODING, use_unicode=DEF_UNICODE_SWITCH
):
    """returns a dict of column names mapped to types"""
    sql = (
    ...
    return [
        x[0]
        for x in executeGetList(
            db, dbCursor, sql, charset=charset, use_unicode=use_unicode
        )
    ]
