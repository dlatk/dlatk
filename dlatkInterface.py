...
def main(fn_args = None):
    ...
    group.add_argument('--emb_model_name', type=str, metavar='NAME', dest='embmodelname', default=dlac.DEF_BERT_MODEL,
                       help='Model weights to use if extracting features.')
    group.add_argument('--emb_msg_aggregation', '--emb_aggregations', type=str, metavar='AGG', nargs='+', dest='embaggregations', default=dlac.DEF_BERT_AGGREGATION,
                       help='Aggregations to use with model(e.g. mean, min, max).')
    group.add_argument('--emb_layer_aggregation', type=str, metavar='AGG', nargs='+', dest='emblayeraggregations', default=dlac.DEF_BERT_LAYER_AGGREGATION,
                       help='Aggregations to use with model (e.g. mean, min, max).')
    group.add_argument('--emb_layers', type=int, metavar='LAYER', nargs='+', dest='emblayerstokeep', default=dlac.DEF_BERT_LAYERS,
                       help='layers from model to keep.')
    group.add_argument('--emb_no_context', action='store_true', dest='embnocontext', default=False,
...
#                feature_type_name = "clc" + str(pmi_filter_thresh).replace('.', '_')
    ...
    if args.embaddfeat:
        if not fe: fe = FE()
        args.feattable = fe.addFeatureTable(modelName = args.embmodelname, aggregations=args.embaggregations, layersToKeep=args.emblayerstokeep, noContext=args.embnocontext, layerAggregations = args.emblayeraggregations, valueFunc = args.valuefunc)
