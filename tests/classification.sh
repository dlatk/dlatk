...
then
    ...
    python ../dlatkInterface.py -d $DATABASE -t $TABLE -c $GROUP_FIELD --group_freq_thresh $GFT --feat_table $FEAT_TABLE --outcome_table $OT --outcomes $OC --nfold_test_classifiers --categories_to_binary $OC --feature_selection $FS --model lr --folds 10 --csv --output_name $OUTPUT
else
    echo "Classification without feature selection..."
    python ../dlatkInterface.py -d $DATABASE -t $TABLE -c $GROUP_FIELD --group_freq_thresh $GFT --feat_table $FEAT_TABLE --outcome_table $OT --outcomes $OC --nfold_test_classifiers --categories_to_binary $OC --model lr --folds 10 --csv --output_name $OUTPUT
fi
